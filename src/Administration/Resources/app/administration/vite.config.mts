...
console.log(picocolors.yellow('# Compiling Administration with Vite configuration'));
...
export default defineConfig(({ command }) => {
    ...
    if (isProd) {
        console.log(picocolors.yellow('# Production mode activated 🚀'));
    }

    // We only load extensions here to display the successfull injection
    const extensions = loadExtensions();
    extensions.forEach((extension) => {
        console.log(picocolors.green(`# Plugin "${extension.name}": Injected successfully`));
    });
